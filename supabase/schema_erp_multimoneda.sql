...
-- -----------------------------------------------------------------------------
-- RPC: crear venta (stock + venta + detalle + caja y/o CXC) — atómico
...
BEGIN
  ...
  IF p_forma_pago = 'contado' AND p_caja_id IS NULL THEN
    RAISE EXCEPTION 'Caja requerida para venta al contado';
  END IF;
...
REVOKE ALL ON FUNCTION public.crear_venta_erp(UUID, TEXT, TEXT, UUID, NUMERIC, NUMERIC, DATE, TEXT, JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.crear_venta_erp(UUID, TEXT, TEXT, UUID, NUMERIC, NUMERIC, DATE, TEXT, JSONB) TO service_role;
...
-- -----------------------------------------------------------------------------
-- RPC: cobro parcial/total de cuenta por cobrar + ingreso a caja (atómico)
-- -----------------------------------------------------------------------------
...
REVOKE ALL ON FUNCTION public.cobrar_cxc_erp(UUID, UUID, UUID, NUMERIC) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.cobrar_cxc_erp(UUID, UUID, UUID, NUMERIC) TO service_role;
